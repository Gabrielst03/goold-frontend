...
function Input({ className, type, rightIcon, showSearchIcon = false, ...props }: InputProps) {
  const hasRightElement = rightIcon || showSearchIcon
    ...
    <div className="relative">
      ...
      {hasRightElement && (
        <div className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground">
          {rightIcon || (showSearchIcon && <Search className="h-4 w-4" />)}
        </div>
      )}
    </div>
  )
